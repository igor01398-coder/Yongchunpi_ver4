...
export const ImageEditor: React.FC<ImageEditorProps> = ({ activePuzzle, onBack, onComplete, onSideMissionProgress, onFieldSolved, initialState, isCompleted }) => {
  ...
  const handleQuizVerify = () => {
    if (!activePuzzle || !activePuzzle.quiz) return;
  ...
  return (
    <div className="flex flex-col h-full w-full max-w-3xl mx-auto bg-slate-50 relative">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6 pb-36 relative z-10">
        ...
        {(activePuzzle && (isPhotoAnalysisMission || (isQuizSolved && activePuzzle.id !== '1'))) && (
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-8 duration-700 fill-mode-forwards">
                ...
                {!originalImage ? (
                    ...
                    <div className="w-full space-y-4">
                         ...
                         {/* COMPLETE BUTTON FOR PHOTO ANALYSIS (When Validated) */}
                         {isPhotoAnalysisMission && validationResult?.isValid && !isCompleted && (
                            <button
                                onClick={handlePreComplete}
                                className="w-full bg-teal-600 hover:bg-teal-500 text-white py-3 rounded-lg font-mono font-bold text-lg uppercase tracking-widest transition-all flex items-center justify-center gap-2 shadow-lg mt-2 animate-in fade-in slide-in-from-bottom-2"
                            >
                                <CheckCircle className="w-5 h-5" />
                             ...
                             <div className="space-y-2">
                                ...
                                <div className="flex gap-2">
                                    <input 
                                        ...
                                        className={`bg-white border border-slate-300 rounded px-3 py-2 text-sm font-mono focus:border-teal-500 focus:outline-none shadow-sm ${isPhotoAnalysisMission ? 'w-full' : 'flex-1'} disabled:bg-slate-100 disabled:text-slate-900 disabled:border-slate-300 disabled:font-bold`}
                                        placeholder={isPhotoAnalysisMission ? "記錄觀察筆記..." : "Describe changes..."}
                                        disabled={isCompleted}
                                    />
                                    {!isPhotoAnalysisMission && (
                                        <button 
                                            ...
                                            className="bg-teal-600 hover:bg-teal-500 disabled:bg-slate-300 text-white px-4 py-2 rounded font-mono font-bold text-xs flex items-center gap-2"
                                        >
                                ...
                                {error && (
                                    <p className="text-xs text-rose-500 font-mono mt-1">{error}</p>
                                )}
                             </div>
                         )}
                    </div>
         ...
         {/* Success Modal */}
         {showSuccessModal && (
            <div className="absolute inset-0 bg-slate-900/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in zoom-in duration-300">
                <div className="bg-white rounded-xl p-8 max-w-sm w-full text-center shadow-2xl relative overflow-hidden">
                    ...
                    <div className="w-20 h-20 bg-teal-100 rounded-full flex items-center justify-center mx-auto mb-6">
                        <PartyPopper className="w-10 h-10 text-teal-600 animate-bounce" />
                    </div>
                    <h2 className="text-2xl font-bold font-mono text-slate-800 mb-2">MISSION ACCOMPLISHED</h2>
                    <p className="text-slate-600 mb-8">Data successfully transmitted to headquarters. Great work, explorer!</p>
                    
                    <button 
                        onClick={handleFinalExit}
                        className="w-full bg-slate-900 hover:bg-slate-800 text-white py-3 rounded-lg font-mono font-bold tracking-wider transition-all"
                    >
                        RETURN TO MAP
                    </button>
                </div>
        ...
        {showCheckGallery && activePuzzle?.referenceCheckImages && (
            <div className="absolute inset-0 bg-slate-900/90 backdrop-blur-md z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
                 <div className="w-full max-w-lg bg-white rounded-xl overflow-hidden flex flex-col max-h-[80vh]">
                    <div className="p-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
                        <h3 className="font-bold font-mono text-slate-700">REFERENCE DATA</h3>
                        <button onClick={() => setShowCheckGallery(false)} className="p-1 hover:bg-slate-200 rounded-full">
                            <X className="w-5 h-5 text-slate-500" />
                        </button>
                    </div>
                    <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-100">
                        {activePuzzle.referenceCheckImages.map((url, idx) => (
                            <div key={idx} className="bg-white p-2 rounded shadow-sm">
                                <img src={url} alt={`Reference ${idx+1}`} className="w-full rounded" />
                                <div className="mt-2 text-[10px] text-slate-400 font-mono text-center">FIG {idx+1}. APPROVED SAMPLE</div>
                            </div>
                        ))}
                    </div>
                 </div>
        ...
        {/* Reference Image Modal (Primary) */}
        {showReferenceImage && activePuzzle?.referenceImage && (
             <div className="absolute inset-0 bg-slate-900/90 backdrop-blur-md z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
                 <div className="w-full max-w-lg bg-white rounded-xl overflow-hidden flex flex-col max-h-[80vh]">
                    <div className="p-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
                        <h3 className="font-bold font-mono text-slate-700">TARGET REFERENCE</h3>
                        <button onClick={() => setShowReferenceImage(false)} className="p-1 hover:bg-slate-200 rounded-full">
                            <X className="w-5 h-5 text-slate-500" />
                        </button>
                    </div>
                    <div className="flex-1 overflow-y-auto p-4 bg-black flex items-center justify-center">
                        <img src={activePuzzle.referenceImage} alt="Reference" className="max-w-full max-h-full object-contain" />
                    </div>
                    {isDriveFolder && (
                        <div className="p-4 bg-slate-50 border-t border-slate-200 text-center">
                            ...
                            >
                                <ExternalLink className="w-4 h-4" /> Open External Folder
                            </a>
                        </div>
                    )}
                 </div>
